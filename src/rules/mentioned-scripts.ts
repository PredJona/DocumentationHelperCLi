...
export class MentionedScriptExistsRule implements DocumentationRule {
  id = 'DOC006';
  async run(context: ProjectContext): Promise<Finding[]> {
    ...
    visit(ast, 'inlineCode', (node: any) => {
      ...
      const match = /(?:npm|yarn|pnpm|bun)\s+run\s+([a-z0-9:-]+)/i.exec(value);
    ...
    visit(ast, 'code', (node: any) => {
      ...
      for (const line of lines) {
        const match = /(?:npm|yarn|pnpm|bun)\s+run\s+([a-z0-9:-]+)/i.exec(line);
        if (match) mentionedScripts.add(match[1]);
      }
    });
